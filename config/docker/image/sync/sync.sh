...
if [ "$1" == "" ]; then
    echo "First parameter should be the relative path of folder to sync"
    exit 1
fi
...
syncSource="${SYNC_SOURCE_BASE_PATH}/${syncFolderRelPath}"
syncDestination="${SYNC_DESTINATION_BASE_PATH}/${syncFolderRelPath}"

# Dump the configuration to the log to aid bug reports.
log_heading "Configuration:"
log_info "SYNC_SOURCE:                  $syncSource"
log_info "SYNC_DESTINATION:             $syncDestination"
...
[ -d "$syncSource" ] || log_error_exit "Source directory does not exist!"
[ -d "$syncDestination" ] || log_error_exit "Destination directory does not exist!"
[[ "$syncSource" != "$syncDestination" ]] || log_error_exit "Source and destination must be different directories!"
...
if [ -z "${SYNC_MAX_INOTIFY_WATCHES}" ]; then
  ...
  file_count="$(find $syncSource | wc -l)"
...
if [[ "$SYNC_NODELETE_SOURCE" == "1" ]]; then
  nodelete="nodeletion=$syncSource"
fi
